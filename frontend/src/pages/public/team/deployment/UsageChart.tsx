...
export function UsageChart(props: UsageChartProps) {
  ...
  const { data: loadedUsage } = useQuery({
    ...
    queryFn: () => clients.deployments.getDeploymentUsage(+teamId!, +deploymentId!, dateFrom, dateTo),
  ...
  return (
    <ResponsiveContainer height={300}>
      <BarChart width={500} height={300} data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
        ...
        <Tooltip />
        <Legend />
        <Bar dataKey="totalCores" stackId="a" fill="#F28E2B" />
        <Bar dataKey="totalMemoryGB" stackId="a" fill="#4E79A7" />
        <Bar dataKey="totalVolumeGB" stackId="a" fill="#59A14F" />
        <Bar dataKey="totalStorageGB" stackId="a" fill="#B07AA1" />
      </BarChart>
    </ResponsiveContainer>
  );
}
